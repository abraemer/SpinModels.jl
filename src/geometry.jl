...
Generate the positions of the spin in the `geometry`. Disordered geometries allow for setting
an `RNG`.
"""
function positions end

# document nspins for Geometry
"""
    nspins(geometry)

Return number of spins the `geometry`.
"""
function nspins(::Geometry)
    error("Not implemented! This should never happen!")
end

"""
    distance_matrix(geometry[, positions]; rng)

Compute the spin-spin distances. Positions are generated by the `geometry` via [`positions`](@ref) or
provided directly. Disordered geometries allow for setting an `RNG`.
"""
...
Represents a 1D chain of `L` spins with regular `spacing`.
...
Represents a 1D chain of `L` spins with random noise on top of regular spaced positions.
Spins are at ``x_i = spacing*i + \\delta`` where ``\\delta \\sim \\mathcal{U}([-\\frac{\\sigma}{2},\\frac{\\sigma}{2}])``
"""
...
"""
    NNGeometry(geometry, k)

See [`NN(::SpinModels.Geometry)`](@ref) and [`nearest_neighbor_from_distances!`](@ref).
"""
...
Non-destructive form of [`nearest_neighbor_from_distances!`](@ref).
See also: [`nearest_neighbor_from_interactions!`](@ref)
"""
nearest_neighbor_from_distances(distance_mat, k=1) = nearest_neighbor_from_distances!(copy(distance_mat), k)
...
function nearest_neighbor_from_distances!(distance_mat, k=1)
    ...
    distance_mat[distance_mat .> d_cutoff'] .= Inf # not nearest nearest_neighbor = infinitely far away
...
## Overwrite defaults
distance_matrix(nng::NNGeometry, positions) = nearest_neighbor_from_distances!(distance_matrix(_parent(nng), positions), nng.k)
